...
export function useUpcomingServices(windowDays = 14, locationId?: string | null): UpcomingServices {
  ...
  const [state, setState] = useState<UpcomingServices>({ total: 0, fire: 0, food: 0, loading: true });

  useEffect(() => {
    if (!orgId) { setState({ total: 0, fire: 0, food: 0, loading: false }); return; }
    let cancelled = false;

    (async () => {
      let q = supabase
        .from('location_service_schedules')
        .select('id, next_due_date, is_active, location_id, service_type_definitions(category)')
      ...
      if (cancelled) return;
      if (error || !data) { setState({ total: 0, fire: 0, food: 0, loading: false }); return; }
      ...
      setState({ total, fire, food, loading: false });
    })();
