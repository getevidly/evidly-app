/**
 * useHotHoldingTabSignals.ts
 *
 * Queries temperature_logs for hot holding equipment at the org's locations.
 *
 * PREDICT = units where next check is due within 30 min (last_reading + 4h <= now + 30m).
 * REDUCE  = units with most recent reading below 135°F (temp_pass = false).
 * PROVE   = hot holding checks logged today where temp_pass = true.
 */
...
import { isHoldingHot } from '../../lib/equipmentHelpers';
import { TEMP_CHECK_INTERVALS } from '../../config/tempConfig';

export interface HotHoldingTabSignals {
  checksDue: number;
  itemsAtRisk: number;
  inRangeToday: number;
...
export function useHotHoldingTabSignals(): HotHoldingTabSignals {
  ...
  const [signals, setSignals] = useState<Omit<HotHoldingTabSignals, 'loading'>>({
    checksDue: 0,
    itemsAtRisk: 0,
    inRangeToday: 0,
    criticalUnits: [],
  ...
  const fetchSignals = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // 1. Get all hot holding equipment
      ...
      const hotEq = (eqData ?? []).filter(eq => isHoldingHot(eq.equipment_type));
      if (hotEq.length === 0) {
        setSignals({ checksDue: 0, itemsAtRisk: 0, inRangeToday: 0, criticalUnits: [], staleUnits: [] });
        setLoading(false);
        return;
      ...
      const hotEqIds = hotEq.map(e => e.id);
      const eqNameMap = new Map(hotEq.map(e => [e.id, e.name]));
      ...
      const now = Date.now();
      const overdueMs = TEMP_CHECK_INTERVALS.HOT_HOLDING_OVERDUE_MINUTES * 60 * 1000;
      ...
      for (const eq of hotEq) {
        ...
        // Critical: below 135°F (temp_pass = false)
        if (!last.temp_pass) {
          itemsAtRisk++;
      ...
      // 3. PROVE: hot holding checks logged today that passed
      ...
      console.warn('[useHotHoldingTabSignals]', e instanceof Error ? e.message : e);
    } finally {
      setLoading(false);
    }
