...
export function getDemoComplianceGaps(): LocationComplianceGap[] {
  return DEMO_LOCATION_JURISDICTIONS.map(location => {
    ...
    if (location.locationName === 'Location 1') { // demo
    ...
    if (location.locationName === 'Location 2') { // demo
      ...
      const gaps: ComplianceGap[] = [
        ...
        { category: 'service', item: 'Fire Suppression System', status: 'overdue', detail: 'Semi-annual fire suppression service is 3 weeks overdue — CFC Ch. 6.07 / NFPA 96 Chapter 11', action: 'Schedule emergency fire suppression service' },
    ...
    // Location 3 — missing several items, overdue on hood cleaning, CFC gaps // demo
    const gaps: ComplianceGap[] = [
