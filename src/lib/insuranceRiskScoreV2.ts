...
export function calculateFireRiskV2(
  ...
  const factors: InsuranceRiskFactor[] = [
    { name: 'Hood cleaning current per NFPA 96 (2024) schedule', score: hoodScore, weight: 0.18, status: factorStatus(hoodScore), detail: hoodScore >= 80 ? 'Hood cleaning current per schedule' : hoodScore >= 40 ? `Hood cleaning due within ${fs.hoodCleaningDaysUntilDue} days` : 'Hood cleaning overdue', reference: 'NFPA 96-2024' },
    ...
    { name: 'Cleaning to bare metal verified', score: bareMetalScore, weight: 0.10, status: factorStatus(bareMetalScore), detail: bareMetalScore >= 90 ? 'Bare metal cleaning documented by vendor' : 'Cleaning verification records incomplete', reference: 'NFPA 96 (2024) Table 12.4' },
