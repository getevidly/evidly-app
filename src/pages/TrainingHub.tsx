...
const CATEGORY_CONFIG: Record<TrainingCategory, { label: string; icon: typeof EvidlyIcon; color: string; bg: string }> = {
  ...
  compliance_ops: { label: 'Compliance Ops', icon: Settings2, color: '#B24A2E', bg: '#fef3c7' },
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 160px), 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: 'Completed', value: trainingEnrollments.filter(e => e.status === 'completed').length, icon: CheckCircle2, color: '#B24A2E' },
      ...
      {/* Filters */}
      <div data-demo-allow style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {courseStats.map(c => (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0', borderBottom: '1px solid #f3f4f6', flexWrap: 'wrap' }}>
              ...
              <div style={{ width: 100, height: 6, background: '#f3f4f6', borderRadius: 3, overflow: 'hidden', flexShrink: 0 }}>
                <div style={{ width: `${c.rate}%`, height: '100%', background: c.rate >= 75 ? '#15803d' : c.rate >= 50 ? '#B24A2E' : '#dc2626', borderRadius: 3 }} />
              </div>
...
function AchievementBadges() {
  const badges = [
    { id: 'fast-learner', name: 'Fast Learner', desc: 'Completed under estimated time', icon: Zap, color: '#B24A2E', bg: '#fffbeb', earned: true },
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <h3 style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <Star size={16} color="#B24A2E" /> Achievement Badges
      </h3>
...
export function TrainingDashboardWidget() {
  ...
  const statusColor = overdue > 0 ? '#dc2626' : inProgress > 0 ? '#B24A2E' : '#15803d';
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: 20, cursor: 'pointer' }}
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#B24A2E' }}>{inProgress}</div>
      ...
      <div style={{ height: 6, background: '#f3f4f6', borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
        ...
        <div style={{ width: `${(inProgress / total) * 100}%`, background: '#B24A2E' }} />
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 16 }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 700, color: '#B24A2E' }}>{recommendedCount}</div>
        ...
        {locationStats.map(loc => (
          <div key={loc.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 16 }}>
            ...
            <div style={{ height: 4, background: '#f3f4f6', borderRadius: 2, overflow: 'hidden', marginTop: 8 }}>
              <div style={{ width: `${loc.pct}%`, height: '100%', background: loc.pct >= 80 ? '#15803d' : loc.pct >= 50 ? '#B24A2E' : '#dc2626', borderRadius: 2 }} />
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => guardAction('chat', 'AI Study Companion', () => toast.info('AI Study Companion (Demo)'))}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: '1px solid #B24A2E', background: '#fffbeb', color: '#92400e', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: typography.family.body }}>
