      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-xl p-5" style={{ background: CARD_BG, border: `1px solid ${CARD_BORDER}`, boxShadow: CARD_SHADOW }}>
            ...
            <div className="space-y-4">
              {report.parameters.map(p => (
                <div key={p.key}>
                  ...
                  {p.type === 'dateRange' && (
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          type="date"
                          className="w-full pl-8 pr-3 py-2 text-sm rounded-xl border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B24A2E]/50 focus-visible:ring-offset-2/30"
                  ...
                  {p.type === 'select' && (
                    <select
                      className="w-full px-3 py-2 text-sm rounded-xl border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B24A2E]/50 focus-visible:ring-offset-2/30"
                      style={{ borderColor: CARD_BORDER, color: NAVY }}
          ...
          <div className="flex items-center gap-3 mt-6 pt-4" style={{ borderTop: `1px solid ${CARD_BORDER}` }}>
            ...
            {generated && (
              <>
                <button onClick={handleExportPdf} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border hover:bg-[#FAF7F0] transition-colors" style={{ borderColor: CARD_BORDER, color: NAVY }}>
                  <Download className="w-3.5 h-3.5" /> PDF
                </button>
                <button onClick={handleExportExcel} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border hover:bg-[#FAF7F0] transition-colors" style={{ borderColor: CARD_BORDER, color: NAVY }}>
                  <FileSpreadsheet className="w-3.5 h-3.5" /> Excel
                </button>
                <button onClick={handlePrint} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border hover:bg-[#FAF7F0] transition-colors" style={{ borderColor: CARD_BORDER, color: NAVY }}>
