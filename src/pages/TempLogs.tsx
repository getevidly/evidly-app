...
import { TemperatureHistoryTab } from '../components/temp-logs/TemperatureHistoryTab';
...
export interface TempCheckCompletion {
  id: string;
  equipment_id: string;
  ...
  logged_retroactively?: boolean;
  retroactive_reason?: string | null;
  retroactive_logged_at?: string | null;
}
...
export function TempLogs() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          log_type,
          menu_item_id,
          logged_retroactively,
          retroactive_reason,
          retroactive_logged_at,
  ...
  const handleRetroactiveSave = async (data: { equipmentId: string; temperature: number; readingTime: string; reason: string; correctiveAction: string | null }): Promise<boolean> => {
    const eq = equipment.find(e => e.id === data.equipmentId);
    if (!eq) return false;

    const isWithinRange = data.temperature >= (eq.min_temp === -Infinity ? -9999 : eq.min_temp) && data.temperature <= eq.max_temp;
    const logType = getLogType(eq.equipment_type);

    const { error } = await supabase.from('temperature_logs').insert({
      facility_id: (eq as any).location_id,
      equipment_id: data.equipmentId,
      input_method: data.readingMethod,
      ...
      logged_retroactively: true,
      retroactive_reason: data.reason,
      retroactive_logged_at: new Date().toISOString(),
    });
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'history' && (
          <TemperatureHistoryTab
            ...
            onHACCPReport={(log) => { setHaccpViolation(log); setShowHACCPReport(true); }}
            onRetroactiveSave={handleRetroactiveSave}
          />
        )}
        ...
        {/* Hot Holding Tab */}
        {activeTab === 'hot_holding' && (
          <>
            <HotHoldingCriticalBanner signals={hotHoldingSignals} />
            <HotHoldingTabSignalStrip signals={hotHoldingSignals} />
            <HoldingActiveStatus variant="hot" driftingUnits={hotDriftingUnits} />
          </>
        )}
