...
const getDemoResponse = (question: string): { text: string; suggestions: string[] } => { // demo
  ...
  if (lowerQ.includes('compliance score') && lowerQ.includes('airport')) { // demo
    ...
    suggestions = ['Why did Airport Cafe score drop?', 'Am I ready for a health inspection?', 'What corrective actions are overdue?']; // demo
  }
...
const DEMO_PAST_CONVERSATIONS: Conversation[] = [ // demo
  {
    id: 'past-1', title: 'Airport Cafe score drop analysis', mode: 'chat', createdAt: Date.now() - 86400000 * 2, // demo
    messages: [
      { id: 'p1-1', role: 'user', content: 'Why did my compliance score drop at Airport Cafe?', timestamp: Date.now() - 86400000 * 2 }, // demo
      { id: 'p1-2', role: 'assistant', content: 'Your compliance score at **Airport Cafe** dropped from **94%** to **87%** over the past 7 days due to 3 missed temperature logs and an expired hood cleaning certificate. The food safety pillar dropped 5 points and facility safety dropped 2 points.', timestamp: Date.now() - 86400000 * 2 + 5000, suggestions: ['Show me the details', 'How to fix it?'] }, // demo
    ],
  },
  {
  ...
  {
    id: 'past-3', title: 'Mock inspection — Downtown Kitchen', mode: 'inspection', createdAt: Date.now() - 86400000 * 7, // demo
    messages: [
      { id: 'p3-1', role: 'assistant', content: 'Starting mock Health Department inspection for Downtown Kitchen. Question 1: What is the current temperature of your walk-in cooler?', timestamp: Date.now() - 86400000 * 7 }, // demo
...
const STARTER_CARDS = [ // demo
  'Why did my compliance score drop at Airport Cafe?', // demo
  'Which vendor documents expire this month?',
  'Am I ready for a health inspection?',
  'What corrective actions are overdue?',
  'Show me temperature trends for Downtown Kitchen', // demo
  'Compare compliance across all my locations',
  'What are the biggest risks at University Dining?', // demo
                      ...
                      <div key={i} style={{ padding: '14px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e5e7eb', marginBottom: '10px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                          ...
                          <div>
                            ...
                            <select style={{ width: '100%', padding: '6px 8px', borderRadius: '6px', border: '1px solid #d1d5db', fontSize: '12px', ...F }}>
                              <option>Downtown Kitchen</option>{/* demo */}
                              <option>Airport Cafe</option>{/* demo */}
                              <option>University Dining</option>{/* demo */}
                            </select>
                          </div>
                        </div>
                      </div>
