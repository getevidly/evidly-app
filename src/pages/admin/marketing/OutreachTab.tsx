...
async function invokeErrorDetail(err: any): Promise<string> {
  try {
...
export default function OutreachTab() {
  ...
  const signOffStep = async (stepNumber: number) => {
    ...
    setActionLoading(null);
    if (error || data?.error) {
      /* The reason lives on the error, not on data: a 422 "content hash
       * mismatch" or a 500 from the drift rewrite both arrive with data null. */
      const detail = error ? await invokeErrorDetail(error) : data?.error;
      flash(`Sign-off failed: ${detail}`);
      return;
    }
