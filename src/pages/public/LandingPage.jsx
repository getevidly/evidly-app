...
export default function LandingPage(){
  ...
  <header style={{background:E.g1,borderBottom:"1px solid "+E.g2,padding:"0 24px",position:"sticky",top:0,zIndex:100}}>
    <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",height:56}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginRight:28}}><Logo s="1.1rem" tagline/></div>
      <nav style={{display:"flex",gap:18,flex:1}}>{["What's Covered","Your County","Kitchen Self Check","Pricing"].map(function(x){return <a key={x} href={"#"+x.toLowerCase().replace(/\s/g,"-")} style={{textDecoration:"none",color:E.g5,fontWeight:500,fontSize:"0.82rem"}}>{x}</a>;})}</nav>
      <div style={{display:"flex",gap:8}}>
        <button onClick={openCalendly} style={Object.assign({},bG,{padding:"7px 14px",fontSize:"0.78rem"})}>Try Demo</button>
        <button onClick={function(){openM("signin");}} style={{padding:"7px 14px",border:"1px solid "+E.g2,borderRadius:8,fontSize:"0.78rem",fontWeight:600,cursor:"pointer",background:E.w,color:E.g6}}>Sign In</button>
  ...
  <section style={{padding:"80px 24px 64px",background:"linear-gradient(160deg,#2a3f6b,"+E.navy+")",textAlign:"center",position:"relative"}}>
    ...
    <div style={{maxWidth:680,margin:"0 auto"}}>
      <h1 style={{fontSize:"clamp(1.8rem,5vw,3rem)",fontWeight:800,lineHeight:1.1,margin:"0 0 16px",color:E.w}}>Lead with Confidence. <span style={{color:E.gold}}>Know Where You Stand.</span></h1>
      <p style={{fontSize:"1.05rem",color:"rgba(255,255,255,0.5)",maxWidth:520,margin:"0 auto 28px",lineHeight:1.7}}>Food safety and facility safety. One clear picture. Updated every day.</p>
  ...
  {/* ═══ WHAT'S COVERED ═══ */}
  <section id="what's-covered" style={{padding:"72px 24px",background:E.w}}>
    <div style={{maxWidth:780,margin:"0 auto"}}>
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <p style={{fontSize:"0.88rem",color:E.g5,maxWidth:520,margin:"0 auto"}}>Most platforms cover one or the other. EvidLY covers both — because that's how your operation actually works.</p>
      </div>
  ...
  {/* ═══ DAY WALKTHROUGH ═══ */}
  <section id="walkthrough" style={{padding:"64px 24px 80px",background:"linear-gradient(160deg,#2a3f6b,"+E.navy+")"}}>
  ...
  {/* ═══ PRICING ═══ */}
  <section id="pricing" style={{padding:"72px 24px",background:E.cream}}>
    <div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20}}>
        <div style={{background:E.cream,borderRadius:16,padding:"32px 24px",border:"2px solid "+E.gold,position:"relative"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7,textAlign:"left",maxWidth:280,margin:"0 auto 16px"}}>{["Scored against your county — not a generic checklist","Know where you stand before an inspector walks in","Every temp, checklist, and record in one place","Covers food safety and facility safety together","Your whole team, no extra cost","If it's not right in 45 days, you pay nothing"].map(function(f){return <div key={f} style={{display:"flex",gap:7,fontSize:"0.82rem",color:E.g6}}><span style={{color:E.gold,fontWeight:700}}>✓</span>{f}</div>;})}</div>
  ...
  <section style={{padding:"28px 24px",background:"#354a73",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
    <p style={{maxWidth:600,margin:"0 auto",textAlign:"center",fontSize:"0.82rem",color:"rgba(255,255,255,0.25)",lineHeight:1.7,fontStyle:"italic"}}>Built to bring together the standards, records, and processes you already rely on — in one dependable place.</p>
  </section>
  ...
  <footer style={{padding:"40px 24px 20px",background:"#354a73"}}>
    <div style={{maxWidth:960,margin:"0 auto",display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:24}}>
      <div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><Logo s="0.95rem" light tagline/></div><div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}><STIcon sz={18}/><span style={{fontSize:"0.72rem",color:"rgba(255,255,255,0.3)"}}><STLogo s="0.72rem" light/></span></div></div>
      <div><h4 style={{fontSize:"0.7rem",fontWeight:700,color:"rgba(255,255,255,0.4)",marginBottom:8,textTransform:"uppercase",letterSpacing:1}}>Product</h4>{[["Food Safety","#what-covered"],["Facility Safety","#what-covered"],["Kitchen Check","/kitchen-check/merced-county"],["Your County","/merced-county"],["Pricing","#pricing"]].map(function(l){return <a key={l[0]} href={l[1]} style={{display:"block",fontSize:"0.78rem",color:"rgba(255,255,255,0.4)",textDecoration:"none",marginBottom:6}}>{l[0]}</a>;})}</div>
