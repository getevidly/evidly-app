/* EvidLY mock dashboard — self-contained sample data (Pacific Restaurant Group,
   four locations). This is the DEMO mock, NOT the real product dashboard.
   Fonts + keyframes are injected by the component itself. */
...
const NAVY='#1C2A3A', CREAM='#F7F1E6', LINE='#EEE7D9', MUTED='#5F6875', META='#6E675A';
const GREEN='#3E5E4B', AMBER='#8A6412', RED='#9E3B32';
const SAGE_TINT='#E3ECE1', AMBER_TINT='#F7EDD3', RED_TINT='#F6E3DF';
const RUST='#B26A43', BLUE='#3E6B8A';
const GOLD='#A08C5A', GOLD_TEXT='#8A6412', WORDMARK='#A17C3B';

const s = (css: string): any => {
  const o = {};
  String(css).split(';').forEach((d) => {
    const i = d.indexOf(':'); if (i < 0) return;
    const prop = d.slice(0, i).trim(); const val = d.slice(i + 1).trim();
    if (!prop || !val) return;
    const key = prop.replace(/^-webkit-/, 'Webkit-').replace(/^-moz-/, 'Moz-').replace(/^-ms-/, 'ms-').replace(/-([a-z])/g, (_, c) => c.toUpperCase());
    o[key] = val;
  });
  return o;
};
...
const TONE = {
  sage:  { fill: '#7FA98B', tint: '#E3ECE1', text: '#3E5E4B', dot: '#547A62' },
  amber: { fill: '#D8A93A', tint: '#F7EDD3', text: '#8A6412', dot: '#D8A93A' },
  red:   { fill: '#9E3B32', tint: '#F6E3DF', text: '#8E332B', dot: '#9E3B32' },
};
const PILLAR = {
...
const ALL_EQ = [
  ...
  return { ...e, state: out ? 'Out of range' : near ? 'Nearing limit' : 'In range',
    pos: Math.max(6, Math.min(94, Math.round(((e.val - e.lo) / span) * 100))), bar: t.fill, bg: t.tint, fg: t.text };
});
const PROOF = [
  { title: 'Hood suppression inspection', meta: 'Filed Mar 3 · Vista Grill · valid 12 months', detail: 'Semi-annual inspection by ACME Fire Protection. Certificate #FS-2291 is on file. EvidLY will remind you 60 days before the September 3 renewal.', doc: 'FS-2291.pdf' },
  { title: 'Cold-holding temperature logs', meta: '30 days complete · all kitchens', detail: '742 automated readings this month, 100% within the 33–40°F safe range. One tap hands a health inspector the complete record.', doc: 'ColdHold-Jul.pdf' },
  { title: 'Food handler certifications', meta: '8 of 8 staff current', detail: 'Every handler is certified through 2026. EvidLY tracks each renewal and warns you before any card lapses.', doc: 'Handlers-2026.pdf' },
];
const UPCOMING = [
  { loc: 'losangeles', title: 'Health Department permit renewal',        days: 12, meta: 'Los Angeles County · $412',  status: 'renew now',    p: 'food' },
  { loc: 'sandiego',   title: 'Pest control service',                    days: 18, meta: 'Ecolab',                      status: 'schedule now', p: 'food' },
  { loc: 'sandiego',   title: 'Fire extinguisher tag renewal',           days: 21, meta: 'Kidde',                       status: 'scheduled',    p: 'fire' },
  { loc: 'longbeach',  title: 'Hood cleaning',                           days: 27, meta: 'Cleaning Pros Plus',          status: 'scheduled',    p: 'fire' },
  { loc: 'losangeles', title: 'Hood cleaning',                           days: 32, meta: 'Cleaning Pros Plus',          status: 'schedule now', p: 'fire' },
  { loc: 'longbeach',  title: 'Certified Food Protection Manager cert',  days: 34, meta: 'Emma Rodriguez',              status: 'in progress',  p: 'food' },
  { loc: 'losangeles', title: 'Semi-annual suppression inspection',      days: 45, meta: 'Fire Systems Co',             status: 'schedule now', p: 'fire' },
].map((u) => ({ ...u, pillar: u.p === 'fire' ? 'Fire safety' : 'Food safety',
...
const TEAM = [ { name: 'María Osuna', role: 'Kitchen manager' }, { name: 'Devin Cho', role: 'Sous chef' }, { name: 'Lauren Pratt', role: 'Shift lead' } ];
const LOC_TABS: [string, string][] = [ ['all', 'Pacific Restaurant Group'], ['losangeles', 'Los Angeles'], ['sandiego', 'San Diego'], ['longbeach', 'Long Beach'] ];
...
const PSE_CONDITIONS = [
  { id: 'kec', name: 'Kitchen exhaust cleaning', current: true }, { id: 'sup', name: 'Fire suppression', current: true },
  { id: 'alarm', name: 'Fire alarm', current: true }, { id: 'sprk', name: 'Fire sprinkler', current: false },
];

const riskData = (loc: string) => {
  const n = loc === 'all' ? 3 : 1;
  const money = (v: number) => '$' + v.toLocaleString('en-US');
  const rng = (l: number, h: number) => money(l * n) + '–' + money(h * n);
  const food = { label: 'Food safety', typ: rng(900, 3500),
    lines: [ { label: 'Foodborne illness', ctx: 'logs · HACCP', range: rng(400, 1800) }, { label: 'Shutdown & reinspection', ctx: 'health dept', range: rng(300, 900) }, { label: 'Reputation recovery', ctx: '', range: rng(200, 800) } ],
    worst: '$250K–$2M+', worstDesc: 'A severe outbreak, with a lawsuit.', covers: 'Covers: receiving, holding & cooling logs · checklists · HACCP',
    annual: '', live: '', reduced: '', ceilingLabel: '', ceiling: '' };
  const fire = { label: 'Fire safety', typ: rng(600, 2600),
    lines: [ { label: 'Fire damage & equipment', ctx: 'NFPA 96 · 17A · 25 · 72', range: rng(300, 1200) }, { label: 'Shutdown & rebuild', ctx: '', range: rng(200, 900) }, { label: 'Reputation recovery', ctx: '', range: rng(100, 500) } ],
    worst: '$150K–$500K+', worstDesc: "A fire your insurance won't cover.", covers: 'Covers: hood cleaning · suppression, alarm & sprinkler checks',
    annual: '', live: '', reduced: '', ceilingLabel: '', ceiling: '' };
  const k = (v: number) => (v >= 1000 ? '$' + (v / 1000).toFixed(v % 1000 === 0 ? 0 : 1) + 'k' : '$' + v);
  const krng = (l: number, h: number) => k(l * n) + '–' + k(h * n);
  food.annual = krng(900, 3500); food.live = krng(450, 1800) + ' live · 2 overdue'; food.reduced = '↓' + krng(450, 1700) + ' reduced by your work';
  food.ceilingLabel = 'An outbreak'; food.ceiling = '$250k–$2m+';
  fire.annual = krng(600, 2600); fire.live = krng(200, 900) + ' live · 1 overdue'; fire.reduced = '↓' + krng(400, 1700) + ' reduced by your work';
  fire.ceilingLabel = "A fire your insurance won't cover"; fire.ceiling = '$150k–$500k+';
  return { pillars: [food, fire], food, fire, totalCompact: krng(1500, 6100), totalTyp: rng(1500, 6100),
    equation: `Food safety (${rng(900, 3500)}) + fire safety (${rng(600, 2600)})`, typeChip: 'Casual dining · ' + (loc === 'all' ? 'Pacific Restaurant Group' : K[loc].name), segment: 'Casual dining' };
};

/* ===== EvidLYDashboardMock (full) ===== */
function EvidLYDashboardMock({ pulse = true, alertTone = 'Advisory', loc: locProp, onLocChange, embedded = false }: any) {
  ...
  const timer = useRef<any>(null); const dispRef = useRef<any>(disp); dispRef.current = disp;
  const k = K[loc];
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const t = Math.min(1, (Date.now() - start) / dur); const e = 1 - Math.pow(1 - t, 3);
      setDisp({ score: Math.round(from.score + (to.score - from.score) * e), days: Math.round(from.days + (to.days - from.days) * e),
        checks: Math.round(from.checks + (to.checks - from.checks) * e), records: Math.round(from.records + (to.records - from.records) * e),
  ...
  const base = [
    { big: fmt(disp.checks), label: 'compliance checks', sub: 'this week · automatic', tipText: 'Automated compliance checks EvidLY ran across your kitchens this week — every requirement, continuously.' },
    { big: fmt(disp.records), label: 'records on file', sub: 'signed, dated, ready', tipText: 'Signed inspections, logs and certificates on file and ready to hand an inspector at any moment.' },
    { big: k.sensors > 0 ? fmt(disp.logs) : '0', label: 'temp logs captured', sub: k.sensors > 0 ? 'from sensors · this week' : 'no sensors · manual only', tipText: 'Temperature readings captured automatically from connected sensors — the backbone of your cold-holding evidence.' },
  ];
  const daysStat = { big: fmt(disp.days), label: 'days covered', sub: 'zero missed requirements', tipText: 'Consecutive days this kitchen has gone with zero missed requirements — EvidLY tracks each kitchen\u2019s streak separately, so it is shown per kitchen, not across all.' };
  ...
  const heroTail = openN > 0 ? 'A receiving-log gap at Vista Grill needs your attention today — everything else is handled.' : 'Everything is handled — nothing needs your attention today.';
  ...
  const pseTotal = pse.length; const pseProven = pse.filter((c) => c.current).length; const pseUnproven = pseTotal - pseProven;
  const gateOpen = pseProven === pseTotal;
  const togglePse = (id: string) => setPse((prev) => prev.map((c) => (c.id === id ? { ...c, current: !c.current } : c)));
  ...
  const nextFire = upcoming.find((u) => u.p === 'fire'); const nextFood = upcoming.find((u) => u.p === 'food');
  const proof = PROOF.map((p, i) => ({ ...p, n: String(i + 1).padStart(2, '0'), open: open === i, chev: open === i ? '–' : '+', onToggle: () => setOpen(open === i ? null : i) }));
  ...
  return (
    <div style={s(`${embedded ? '' : 'min-height:100vh;'}background:#F7F1E6;font-family:'Instrument Sans',system-ui,sans-serif;color:#1C2A3A;position:relative;`)}>
      <style>{CSS}</style>
      <div style={s(embedded ? 'padding:22px 22px 30px;display:flex;flex-direction:column;gap:24px;' : 'max-width:1180px;margin:0 auto;padding:34px 40px 90px;display:flex;flex-direction:column;gap:30px;')}>
              ...
              <div style={{ ...s('position:absolute;inset:24px;border-radius:50%;box-shadow:0 22px 48px -28px rgba(84,122,98,.75);'), background: ringBg }}>
                <div style={s('position:absolute;inset:19px;background:#F7F1E6;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;')}>
                  <div style={s("font-family:'Spectral',serif;font-weight:500;color:#1C2A3A;line-height:1;font-variant-numeric:tabular-nums;display:flex;align-items:baseline;gap:2px;")}>
                    <span style={s('font-size:52px;')}>{disp.score}</span><span style={s('font-size:24px;')}>%</span>
                  </div>
                  <div style={s('position:relative;display:inline-flex;align-items:center;gap:4px;margin-top:6px;white-space:nowrap;max-width:118px;')} onMouseEnter={() => setTip('ring')} onMouseLeave={offTip}>
        ...
        <div style={s('background:#fff;border:1px solid #EEE7D9;border-top:2px solid #A08C5A;border-radius:16px;padding:22px 26px;box-shadow:0 1px 2px rgba(28,42,58,.03),0 18px 40px -36px rgba(28,42,58,.5);')}>
          <div style={s('display:flex;justify-content:space-between;align-items:baseline;gap:16px;flex-wrap:wrap;')}>
            ...
            <span style={{ ...s("font-family:'IBM Plex Mono',monospace;font-size:11px;letter-spacing:.04em;"), color: gateOpen ? TONE.sage.text : '#6E675A' }}>{gateOpen ? `ALL ${pseTotal} CONDITIONS CURRENT` : `${pseProven} OF ${pseTotal} CONDITIONS CURRENT`}</span>
          </div>
          ...
          {[risk.food, risk.fire].map((p) => (
            <div key={p.label} style={s('border-top:1px solid #F0EADC;padding:13px 0;margin-top:12px;')}>
              <div style={s('display:flex;justify-content:space-between;align-items:baseline;')}>
                ...
                <span style={s("font-family:'Spectral',serif;font-size:20px;font-weight:500;color:#1C2A3A;")}>{p.annual}<span style={s('font-size:12px;color:#6E675A;font-weight:400;')}>/yr</span></span>
              </div>
          ...
          <div style={s('border-top:1px solid #D6CFC0;padding-top:13px;margin-top:2px;display:flex;justify-content:space-between;align-items:baseline;')}>
            ...
            <span style={s("font-family:'Spectral',serif;font-size:24px;font-weight:600;color:#1C2A3A;")}>{risk.totalCompact}<span style={s('font-size:12px;color:#6E675A;font-weight:400;')}>/yr</span></span>
          </div>
          <div style={{ ...s('margin-top:16px;border-radius:10px;padding:14px 16px;'), background: gateOpen ? TONE.sage.tint : TONE.red.tint, border: `1px solid ${gateOpen ? '#BDD3C1' : '#E5B9B2'}` }}>
            <div style={{ ...s("font-family:'IBM Plex Mono',monospace;font-size:10.5px;letter-spacing:.1em;text-transform:uppercase;margin-bottom:11px;"), color: gateOpen ? TONE.sage.text : TONE.red.text }}>If things go wrong once</div>
            <div style={s('display:flex;justify-content:space-between;align-items:baseline;gap:10px;')}>
              ...
              <span style={{ ...s('font-size:15px;font-weight:600;white-space:nowrap;'), color: gateOpen ? TONE.sage.text : TONE.red.text }}>
                {gateOpen ? (<React.Fragment><span style={s('text-decoration:line-through;font-weight:400;opacity:.6;')}>{risk.fire.ceiling}</span>&nbsp;&nbsp;Closed</React.Fragment>) : risk.fire.ceiling}
              </span>
            </div>
            <div style={{ ...s('margin-top:6px;font-size:12px;line-height:1.5;'), color: gateOpen ? TONE.sage.text : TONE.red.text }}>
              {gateOpen ? 'Every safeguard your policy names has a current, sealed record. This ground for denial is closed.' : `${pseUnproven === 1 ? "One safeguard your policy names doesn't have a current record" : `${pseUnproven} safeguards your policy names don't have a current record`} — so this stands in full. Three of four earns nothing.`}
            </div>
            <div style={{ ...s('margin-top:12px;padding-top:12px;display:flex;justify-content:space-between;align-items:baseline;'), borderTop: `1px solid ${gateOpen ? '#BDD3C1' : '#E5B9B2'}` }}>
            ...
            <div style={{ ...s('margin-top:6px;font-size:12px;line-height:1.5;'), color: gateOpen ? TONE.sage.text : TONE.red.text }}>Your records are your defense here. They don't take this off the table.</div>
          </div>
          <div style={s('margin-top:13px;font-size:11px;line-height:1.5;color:#6E675A;')}>Illustrative figures, conservative basis. "If things go wrong once" is a one-time ceiling — not a yearly cost. EvidLY reads and identifies what your policy requires. It does not determine coverage.</div>
        ...
        <div style={s('background:#fff;border:1px dashed #D8CDB4;border-radius:14px;padding:16px 20px;')}>
          ...
          <div style={s('font-size:12.5px;color:#1C2A3A;margin:6px 0 12px;line-height:1.5;')}>Toggle a safeguard to watch the gate. Any single unproven condition holds the full ceiling — proving three of four earns nothing.</div>
          {pse.map((c) => (
            <label key={c.id} style={s('display:flex;align-items:center;gap:10px;padding:7px 0;cursor:pointer;font-size:13px;color:#1C2A3A;')}>
              <input type="checkbox" checked={c.current} onChange={() => togglePse(c.id)} style={{ width: 15, height: 15, accentColor: '#1C2A3A', cursor: 'pointer' }} />
              <span>{c.name}</span>
              <span style={{ ...s('margin-left:auto;font-size:11px;'), color: c.current ? TONE.sage.text : TONE.red.text }}>{c.current ? 'current record' : 'no current record'}</span>
            </label>
          ))}
        </div>
          ...
          <div onClick={() => setRiskOpen(false)} style={s('position:fixed;inset:0;z-index:100;background:rgba(28,42,58,.5);display:flex;align-items:flex-start;justify-content:center;padding:44px 20px;overflow:auto;')}>
            <div onClick={(e) => e.stopPropagation()} style={s('width:100%;max-width:720px;background:#F7F1E6;border-radius:20px;box-shadow:0 40px 90px -30px rgba(28,42,58,.65);overflow:hidden;')}>
              ...
              <div style={s('padding:22px 30px 30px;display:flex;flex-direction:column;gap:18px;')}>
                <div style={s('background:#F3EAD6;border-radius:10px;padding:12px 15px;font-size:12.5px;line-height:1.5;color:#7A6A45;')}>Illustrative figures, conservative basis. "Worst case" is a one-time ceiling if things go wrong — not a yearly cost.</div>
                {risk.pillars.map((p) => (
                  <div key={p.label} style={s('background:#fff;border:1px solid #EEE7D9;border-radius:16px;padding:22px 24px;')}>
                    ...
                    <div style={s('margin-top:16px;border-top:1px solid #F0EADC;')}>
                      {p.lines.map((ln) => (
                        <div key={ln.label} style={s('display:flex;justify-content:space-between;align-items:baseline;gap:12px;padding:10px 0;border-bottom:1px solid #F5EFE2;')}>
                          <span style={s('font-size:13.5px;color:#4A5566;')}>{ln.label}{ln.ctx && <span style={s("color:#6E675A;font-size:11px;font-family:'IBM Plex Mono',monospace;")}> · {ln.ctx}</span>}</span>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ ...s('margin-top:16px;border-radius:12px;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;gap:12px;'), background: closed ? TONE.sage.tint : '#F4F1E8', border: `1px solid ${closed ? '#BDD3C1' : '#E7E0D2'}` }}>
                          <div>
                            ...
                            <div style={{ ...s('font-size:13px;margin-top:3px;font-style:italic;'), color: closed ? TONE.sage.text : '#6E6656' }}>{closed ? 'Every safeguard your policy names has a current record. This ground for denial is closed.' : p.worstDesc}</div>
                          </div>
                          <div style={{ ...s("font-family:'Spectral',serif;font-size:22px;font-weight:500;white-space:nowrap;"), color: closed ? TONE.sage.text : '#1C2A3A' }}>
                            {closed ? <React.Fragment><span style={s('text-decoration:line-through;font-weight:400;opacity:.6;')}>{p.worst}</span>&nbsp;&nbsp;Closed</React.Fragment> : p.worst}
                          </div>
                        </div>
                      );
                    })()}
                ...
                <div style={s('background:#1C2A3A;border-radius:16px;padding:24px 26px;color:#EDE7DA;')}>
                  ...
                  <div style={s('font-size:12px;line-height:1.55;color:#9AA6B4;margin-top:14px;border-top:1px solid rgba(255,255,255,.12);padding-top:14px;')}>This is money at risk, added up. It's not a verdict on your kitchen, and it doesn't replace your county's inspection. Worst-case figures are shown separately for each area and aren't added together.</div>
                </div>
                <div style={s('font-size:11.5px;line-height:1.5;color:#6E675A;')}>Basis: USDA ERS · CDC NORS · Bartsch et al. (food) · NFPA / Campbell · ISO CP 04 11 (fire). Figures shown at the conservative end.</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={s('position:relative;')}>
          ...
          {statTipOpen && (<div style={{ ...s('position:absolute;top:calc(100% + 8px);transform:translateX(-50%);z-index:30;width:262px;background:#1C2A3A;color:#EDE7DA;font-size:12px;line-height:1.55;padding:13px 15px;border-radius:10px;box-shadow:0 18px 40px -18px rgba(28,42,58,.7);'), left: statLefts[tip] }}>{stats[tip].tipText}</div>)}
        </div>
          ...
          {openN > 0 && (
            <div style={{ ...s('display:flex;background:#FCFAF4;border:1px solid #E9E0CC;border-radius:14px;overflow:hidden;'), borderLeft: `3px solid ${alertT.fill}` }}>
              <div style={s('padding:24px 26px;flex:1;')}>
                ...
                <p style={s('font-size:14.5px;line-height:1.6;color:#4A5566;margin:9px 0 0;max-width:560px;')}>A receiving log hasn't been recorded at Vista Grill since Thursday. EvidLY flagged it and routed it to you. Log today's delivery to close it — or hand it to a team member.</p>
              </div>
                    ...
                    <React.Fragment>
                      ...
                      {assignOpen && (
                        <div style={s('margin-top:8px;background:#fff;border:1px solid #E4DBC8;border-radius:11px;box-shadow:0 16px 34px -22px rgba(28,42,58,.5);overflow:hidden;')}>
                          {TEAM.map((m) => (
                            <button key={m.name} className="ev-soft" onClick={() => { setAssignedTo(m.name); setAssignOpen(false); }} style={s("width:100%;text-align:left;background:none;border:none;padding:10px 14px;cursor:pointer;font-family:'Instrument Sans',sans-serif;display:flex;flex-direction:column;gap:1px;")}>
                ...
                <div style={s('display:flex;align-items:center;gap:11px;')}>
                  ...
                  <div>
                    <div style={s('display:flex;align-items:center;gap:6px;')}>
                      ...
                      <span onMouseEnter={() => setTip('fire')} onMouseLeave={offTip} style={s("position:relative;width:15px;height:15px;border-radius:50%;border:1px solid #C9BFA8;color:#6E675A;font-size:9px;display:inline-flex;align-items:center;justify-content:center;cursor:help;font-family:'IBM Plex Mono',monospace;")}>i{tip === 'fire' && (<span style={s("position:absolute;top:calc(100% + 9px);left:-10px;z-index:30;width:280px;background:#1C2A3A;color:#EDE7DA;font-size:12px;font-weight:400;line-height:1.55;padding:13px 15px;border-radius:10px;box-shadow:0 18px 40px -18px rgba(28,42,58,.7);text-align:left;font-family:'Instrument Sans',sans-serif;")}>{FIRE_TIP}</span>)}</span>
                    </div>
                    <div style={s('font-size:12px;color:#5F6875;margin-top:1px;')}>California Fire Code</div>
                  </div>
                </div>
                <span style={s('display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:600;background:#E3ECE1;color:#3E5E4B;padding:5px 11px;border-radius:999px;')}><span style={s('width:6px;height:6px;border-radius:50%;background:#547A62;')} />On track</span>
              </div>
              <div style={s('display:flex;flex-wrap:wrap;gap:6px;')}>{['NFPA 10', '17A', '25', '72', '96'].map((c) => (<span key={c} style={{ ...s("font-family:'IBM Plex Mono',monospace;font-size:10.5px;padding:3px 8px;border-radius:6px;"), background: PILLAR.fire.tint, color: PILLAR.fire.text }}>{c}</span>))}</div>
              <div style={s('height:6px;border-radius:999px;background:#F0EADC;overflow:hidden;')}><div style={s('height:100%;width:100%;background:#7FA98B;')} /></div>
              <div style={s('display:flex;justify-content:space-between;font-size:12.5px;color:#5F6875;')}><span style={s('color:#4A5566;font-weight:500;')}>9 of 9 requirements current</span><span>{nextFire ? `Next: ${nextFire.title.toLowerCase()} · ${nextFire.days} days` : 'Nothing due'}</span></div>
            </div>
            <div className="ev-card" style={s('background:#fff;border:1px solid #EEE7D9;border-radius:16px;padding:24px;display:flex;flex-direction:column;gap:16px;box-shadow:0 1px 2px rgba(28,42,58,.03),0 18px 40px -36px rgba(28,42,58,.5);transition:transform .18s ease,box-shadow .18s ease;')}>
              <div style={s('display:flex;justify-content:space-between;align-items:flex-start;')}>
                <div style={s('display:flex;align-items:center;gap:11px;')}>
                  ...
                  <div>
                    <div style={s('display:flex;align-items:center;gap:6px;')}>
                      ...
                      <span onMouseEnter={() => setTip('food')} onMouseLeave={offTip} style={s("position:relative;width:15px;height:15px;border-radius:50%;border:1px solid #C9BFA8;color:#6E675A;font-size:9px;display:inline-flex;align-items:center;justify-content:center;cursor:help;font-family:'IBM Plex Mono',monospace;")}>i{tip === 'food' && (<span style={s("position:absolute;top:calc(100% + 9px);left:-10px;z-index:30;width:280px;background:#1C2A3A;color:#EDE7DA;font-size:12px;font-weight:400;line-height:1.55;padding:13px 15px;border-radius:10px;box-shadow:0 18px 40px -18px rgba(28,42,58,.7);text-align:left;font-family:'Instrument Sans',sans-serif;")}>{FOOD_TIP}</span>)}</span>
                    </div>
                    <div style={s('font-size:12px;color:#5F6875;margin-top:1px;')}>California Retail Food Code</div>
                  </div>
                </div>
                <span style={{ ...s('display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:600;padding:5px 11px;border-radius:999px;'), background: food.pillBg, color: food.pillFg }}><span style={{ ...s('width:6px;height:6px;border-radius:50%;'), background: food.dot }} />{food.status}</span>
              </div>
              <div style={s('display:flex;flex-wrap:wrap;gap:6px;')}>{['Receiving', 'Temperature & logging lifecycle', 'Sanitation', 'Handlers'].map((c) => (<span key={c} style={{ ...s("font-family:'IBM Plex Mono',monospace;font-size:10.5px;padding:3px 8px;border-radius:6px;"), background: PILLAR.food.tint, color: PILLAR.food.text }}>{c}</span>))}</div>
              <div style={s('height:6px;border-radius:999px;background:#F0EADC;overflow:hidden;')}><div style={{ ...s('height:100%;'), width: food.width, background: food.bar }} /></div>
              <div style={s('display:flex;justify-content:space-between;font-size:12.5px;color:#5F6875;')}><span style={s('color:#4A5566;font-weight:500;')}>{food.current}</span><span style={{ color: food.nextColor }}>{food.next ?? (nextFood ? `Next: ${nextFood.title.toLowerCase()} · ${nextFood.days} days` : 'Nothing due')}</span></div>
            </div>
          </div>
        </div>
                ...
                {p.open && (
                  <div style={s('padding:0 4px 20px 42px;display:flex;flex-direction:column;gap:13px;')}>
                    ...
                    <div style={s('display:flex;align-items:center;gap:12px;')}>
                      ...
                      <a href="#" style={s('font-size:13px;font-weight:600;color:#1C2A3A;')}>Download evidence →</a>
                    </div>
                  </div>
                )}
              </div>
              ...
              {equipment.map((e) => (
                <div key={e.name} style={s('background:#fff;border:1px solid #EEE7D9;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:15px;')}>
                  ...
                  <div style={s('display:flex;justify-content:space-between;align-items:center;')}>
                    <span style={s("display:inline-flex;align-items:center;gap:5px;font-family:'IBM Plex Mono',monospace;font-size:10px;letter-spacing:.05em;color:#3E5E4B;")}><span style={s('width:5px;height:5px;border-radius:50%;background:#547A62;')} />Sensor</span>
                  ...
                  <div>
                    ...
                    <div style={s("display:flex;justify-content:space-between;margin-top:9px;font-family:'IBM Plex Mono',monospace;font-size:10px;color:#6E675A;")}><span>{e.lo}°</span><span>{e.hi}°</span></div>
                  </div>
                </div>
              ))}
            </div>
...
/* ===== DemoDashboardPublic — modal gate wrapper ===== */
export function DemoDashboardPublic() {
  const [loc, setLoc] = useState('all');
  const [open, setOpen] = useState(true);
  return (
    <div style={{ minHeight: '100vh', background: '#EFE8DA', fontFamily: SANS, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '22px 14px' }}>
      {open ? (
        <div style={{ width: '100%', maxWidth: 1060, background: '#fff', borderRadius: 14, boxShadow: '0 40px 100px -30px rgba(28,42,58,.65)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '94vh' }}>
          <div style={{ background: NAVY, padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' as const }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontFamily: BRAND, fontWeight: 800, fontSize: 20, letterSpacing: '-.01em' }}>
                <span style={{ color: GOLD }}>E</span><span style={{ color: '#fff' }}>vid</span><span style={{ color: GOLD }}>LY</span>
              </span>
              <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '.14em', color: 'rgba(255,255,255,.5)', textTransform: 'uppercase' as const }}>Dashboard demo</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
              <div style={{ display: 'flex', gap: 2, border: '1px solid rgba(255,255,255,.2)', overflowX: 'auto' as const }}>
                {LOC_TABS.map(([id, label]) => {
                  const a = loc === id;
                  return (
                    <button key={id} onClick={() => setLoc(id)} style={{ fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', padding: '7px 12px', whiteSpace: 'nowrap' as const, background: a ? GOLD : 'transparent', color: a ? '#fff' : 'rgba(255,255,255,.7)', fontFamily: a ? SERIF : SANS }}>{label}</button>
                  );
                })}
              </div>
              <button onClick={() => setOpen(false)} title="Close" style={{ background: 'rgba(255,255,255,.12)', border: 'none', color: '#fff', width: 30, height: 30, borderRadius: 6, cursor: 'pointer', fontSize: 16, lineHeight: 1, flexShrink: 0 }}>×</button>
            </div>
          </div>
          <div style={{ overflowY: 'auto' as const, background: CREAM }}>
            <EvidLYDashboardMock embedded={true} loc={loc} onLocChange={setLoc} pulse={true} />
          </div>
        </div>
      ) : (
        <div style={{ marginTop: '22vh', textAlign: 'center' as const }}>
          <button onClick={() => setOpen(true)} style={{ background: NAVY, color: '#fff', border: 'none', borderRadius: 8, padding: '14px 28px', fontFamily: SANS, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Open the dashboard demo →</button>
        </div>
      )}
    </div>
  );
