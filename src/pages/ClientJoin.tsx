...
export function ClientJoin({ previewOnly = false }: { previewOnly?: boolean }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: CREAM, fontFamily: SANS }}>
      ...
      {/* ══════ TABLET RAIL — stacks on top (640–1023 px) ══════ */}
      <div className="hidden sm:block lg:hidden" style={{ background: NAVY, padding: '28px 22px' }}>
        {railInner}
      </div>
      ...
      <div className="lg:ml-[290px]" style={{ paddingBottom: 80 }}>
        ...
        {/* ── 1. SAMPLE BADGE BAR (hidden on phones) ── */}
        <div className="hidden sm:block" style={{ background: '#FBF9F2', borderBottom: `1px solid ${LINE}`, padding: '14px 28px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' as const }}>
        ...
        {/* ── 2. LOCATION TABS + SHARE (hidden on phones) ── */}
        <div className="hidden sm:block" style={{ display: 'flex', alignItems: 'center' }}>
