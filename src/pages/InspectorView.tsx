...
//   - Cert panel: 3 CFPMs, 8 food handlers, 4 fire safety records
    ...
    <>
      ...
      <div className="min-h-screen bg-cream">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          {isDemoMode && (
            <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4 print:hidden">
              <p className="text-xs font-semibold text-[#1E2D4D]/50 uppercase tracking-wide mb-2">
                Scenario
              </p>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold tracking-tight text-[#1E2D4D]">
              ...
              <div className="flex items-center gap-2 print:hidden">
                <button onClick={handlePrint}
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-[#1E2D4D] bg-white border border-[#1E2D4D]/10 rounded-xl hover:bg-[#eef4f8] transition-colors">
          ...
          <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-6">
            ...
            {data.lastInspection ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#1E2D4D]/80">{data.lastInspection.agency}</p>
                  </div>
                </div>
                {data.lastInspection.topViolations.length > 0 && (
                  <div>
                    <p className="text-xs text-[#1E2D4D]/50 uppercase mb-1">Top Violations Cited</p>
              ...
              <p className="text-sm text-[#1E2D4D]/50 italic">No prior inspection history for this location.</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-xs font-bold text-[#1E2D4D]/80 uppercase">Open Corrective Actions</h4>
              </div>
              {data.openCorrectiveActions.length > 0 ? (
                <>
                  <p className="text-2xl font-bold tracking-tight text-amber-700 mb-2">{data.openCorrectiveActions.length}</p>
                  <ul className="space-y-1.5">
                    {data.openCorrectiveActions.slice(0, 3).map((ca, i) => (
                      <li key={i} className="text-xs text-[#1E2D4D]/80 flex items-start gap-1.5">
                        ...
                        <span>{ca.title} <span className="text-[#1E2D4D]/30">&middot; due {ca.dueDate}</span></span>
                      </li>
                    ))}
                  </ul>
                </>
            ...
            <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4">
              <div className="flex items-center gap-2 mb-2">
                <Wrench className="h-4 w-4 text-[#1E2D4D]/50" />
                <h4 className="text-xs font-bold text-[#1E2D4D]/80 uppercase">Overdue Equipment</h4>
              </div>
              {data.overdueEquipment.length > 0 ? (
                <>
                  <p className="text-2xl font-bold tracking-tight text-red-700 mb-2">{data.overdueEquipment.length}</p>
                  <ul className="space-y-1.5">
                    {data.overdueEquipment.map((eq, i) => (
                      <li key={i} className="text-xs text-[#1E2D4D]/80">
                        {eq.name} <span className="text-[#1E2D4D]/30">&middot; due {eq.nextDue}</span>
                      </li>
                    ))}
                  </ul>
                </>
            ...
            <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-xs font-bold text-[#1E2D4D]/80 uppercase">HACCP CCP Alerts</h4>
              </div>
              {data.haccpAlerts.length > 0 ? (
                <>
                  <p className="text-2xl font-bold tracking-tight text-red-700 mb-2">{data.haccpAlerts.length}</p>
                  <ul className="space-y-1.5">
                    {data.haccpAlerts.map((alert, i) => (
                      <li key={i} className="text-xs text-[#1E2D4D]/80">
                        <span className="font-medium text-red-700">{alert.ccp}:</span> {alert.description}
                        <span className="text-[#1E2D4D]/30"> &middot; {alert.detectedAt}</span>
                      </li>
                    ))}
                  </ul>
                </>
                    ...
                    <button
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="ml-2 text-xs text-[#1E2D4D]/30 font-normal">{section.calCode}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        {issues === 0 ? (
                        ...
                        {section.items.map((item, idx) => (
                          <div key={idx}
                            ...
                            <div className="min-w-0 flex-1 space-y-1.5">
                              ...
                              <p className="text-xs text-[#1E2D4D]/30 italic">{item.detail}</p>
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="hidden print:block text-center text-xs text-[#1E2D4D]/50 pt-4 border-t border-[#1E2D4D]/10">
      ...
      <Modal isOpen={showCertPanel} onClose={() => setShowCertPanel(false)} size="xl" className="print:shadow-none print:rounded-none print:max-h-none">
          <div className="flex flex-col max-h-[85vh] print:max-h-none">
            ...
            <div className="overflow-y-auto px-6 py-4 space-y-5">
              ...
              <div>
                ...
                {data.certifications.cfpms.length > 0 ? (
                      ...
                      <tbody>
                        {data.certifications.cfpms.map((c, i) => (
                          <tr key={i} className="border-b border-[#1E2D4D]/5 last:border-0">
                            ...
                            <td className={`px-3 py-2 ${c.status === 'Expiring' ? 'text-amber-600 font-medium' : 'text-[#1E2D4D]/70'}`}>{c.expires}</td>
                  ...
                  <p className="text-sm text-[#1E2D4D]/50 italic">No CFPM records on file.</p>
                )}
                <p className="text-xs text-[#1E2D4D]/50 mt-1">CalCode §113947.1 — at least one CFPM required per establishment during operating hours</p>
              </div>
              {/* Food Handlers */}
              <div>
                ...
                {data.certifications.foodHandlers.length > 0 ? (
                      ...
                      <tbody>
                        {data.certifications.foodHandlers.map((r, i) => (
                          <tr key={i} className="border-b border-[#1E2D4D]/5 last:border-0">
                            ...
                            <td className={`px-3 py-2 ${r.ok ? 'text-[#1E2D4D]/70' : 'text-red-600 font-medium'}`}>{r.exp}</td>
                  ...
                  <p className="text-sm text-[#1E2D4D]/50 italic">No food handler records on file.</p>
                )}
                <p className="text-xs text-[#1E2D4D]/50 mt-1">CalCode §113948 / SB 476 — food handler certification required within 30 days of hire</p>
              </div>
              {/* Fire Safety */}
              <div>
                ...
                {data.certifications.facilitySafety.length > 0 ? (
                      ...
                      <tbody>
                        {data.certifications.facilitySafety.map((r, i) => (
                          <tr key={i} className="border-b border-[#1E2D4D]/5 last:border-0">
                            ...
                            <td className="px-3 py-2 text-[#1E2D4D]/70">{r.next}</td>
                  ...
                  <p className="text-sm text-[#1E2D4D]/50 italic">No fire safety training records on file.</p>
                )}
                <p className="text-xs text-[#1E2D4D]/50 mt-1">OSHA 29 CFR 1910.157 / NFPA 10 — annual fire extinguisher training required</p>
              </div>
            </div>
            <div className="px-6 py-3 border-t border-[#1E2D4D]/10 flex items-center justify-between text-xs text-[#1E2D4D]/30 print:border-t-2">
              <span>
                ...
                {' '}&middot; {data.certifications.facilitySafety.length} fire safety record{data.certifications.facilitySafety.length !== 1 ? 's' : ''}
              </span>
