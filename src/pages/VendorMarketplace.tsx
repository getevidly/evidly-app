...
function TierBadge({ tier }: { tier: MarketplaceTier }) {
  ...
  > = {
    ...
    certified: {
      bg: 'bg-gray-100 text-[#1E2D4D]/80 border border-[#1E2D4D]/10',
      icon: <EvidlyIcon size={12} />,
      label: 'Certified',
    },
    preferred: {
...
function StarRating({ rating, count }: { rating: number; count?: number }) {
  return (
    <div className="flex items-center">
      ...
      <span className="text-sm font-medium text-[#1E2D4D]/80 ml-1">
        {rating.toFixed(1)}
      </span>
      {count !== undefined && (
        <span className="text-xs text-[#1E2D4D]/50 ml-1">({count})</span>
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#f7f9fb]">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="bg-gradient-to-r from-[#1E2D4D] to-[#2c5f7f] rounded-2xl p-6 sm:p-8 mb-6">
          <h1 className="text-2xl font-bold tracking-tight text-white">Vendor Marketplace</h1>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 text-sm cursor-pointer transition-colors whitespace-nowrap min-h-[44px] ${
                  activeSection === sec.id ? 'border-b-2 font-semibold' : 'text-[#1E2D4D]/50 hover:text-gray-700'
                }`}
          ...
          <div>
            ...
            <div className="relative max-w-md mb-6">
              ...
              <input
                ...
                className="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-sm text-gray-900 placeholder-[#1E2D4D]/30 border border-[#1E2D4D]/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#A08C5A]/50 focus-visible:ring-offset-2/20"
              />
            </div>

            {loading ? (
              <div className="flex items-center justify-center py-12">
                <Loader2 className="h-6 w-6 animate-spin text-[#1E2D4D]/30" />
                <span className="ml-2 text-sm text-[#1E2D4D]/50">Loading vendors...</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredVendors.map(vendor => (
                  <div
                    ...
                    className="bg-white rounded-xl border border-[#1E2D4D]/10 hover:shadow-md transition-shadow cursor-pointer overflow-hidden border-l-4"
                  ...
                  >
                    <div className="p-5">
                      ...
                      {vendor.description && (
                        <p className="text-xs text-[#1E2D4D]/50 mb-3 line-clamp-2">{vendor.description}</p>
                      )}
              ...
              <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-12 text-center">
                ...
                <p className="text-[#1E2D4D]/70 font-medium">No approved vendors yet</p>
            ...
            <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#1E2D4D]/15 rounded-lg text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#A08C5A]/50 focus-visible:ring-offset-2/20"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#1E2D4D]/80 mb-1">Contact Name</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#1E2D4D]/80 mb-1">Website</label>
                    <input
                      type="url"
                ...
                <div>
                  <label className="block text-sm font-medium text-[#1E2D4D]/80 mb-1">Service Category</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-[#1E2D4D]/80 mb-1">Description</label>
                  <textarea
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                <button
                  ...
                  className="px-4 py-2 border border-[#1E2D4D]/15 rounded-lg text-sm font-medium text-[#1E2D4D]/80 hover:bg-gray-50 min-h-[44px]"
                >
                  Cancel
                </button>
                <button
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {pendingVendors.map(vendor => (
                  <div key={vendor.id} className="bg-white rounded-xl border border-[#1E2D4D]/10 p-6">
                    <div className="flex items-start justify-between flex-wrap gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        {vendor.contact_name && <p className="text-sm text-[#1E2D4D]/70">Contact: {vendor.contact_name}</p>}
                        {vendor.email && <p className="text-sm text-[#1E2D4D]/50">{vendor.email}</p>}
                        {vendor.phone && <p className="text-sm text-[#1E2D4D]/50">{vendor.phone}</p>}
                        {vendor.description && <p className="text-sm text-[#1E2D4D]/50 mt-2">{vendor.description}</p>}
              ...
              <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-12 text-center">
                ...
                <p className="text-[#1E2D4D]/70 font-medium">No pending approvals</p>
                <p className="text-sm text-[#1E2D4D]/50 mt-1">All vendor invitations have been reviewed.</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="p-6">
        ...
        >
          ...
          <div className="relative max-w-2xl">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-sm text-gray-900 placeholder-[#1E2D4D]/30 focus-visible:outline-none focus-visible:ring-2 focus:ring-white/40"
            />
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {marketplaceCategories.map((cat) => {
            ...
            const bgColor = categoryBgColors[cat.icon] || 'bg-gray-100 text-[#1E2D4D]/70';
        ...
        {/*  Filter Bar                                               */}
        {/* -------------------------------------------------------- */}
        <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4 mb-6">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 text-sm font-medium text-[#1E2D4D]/80">
            ...
            <div className="relative">
              <MapPin className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[#1E2D4D]/30" />
              <input
                type="text"
        ...
        {filteredVendors.length > 0 ? (
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {vendor.subcategories.slice(0, 3).map((sub) => (
                      <span
                        key={sub}
                        className="bg-[#1E2D4D]/5 rounded-full px-2.5 py-0.5 text-xs font-medium text-[#1E2D4D]/70"
                      >
                        {sub}
                      </span>
                    ))}
                    {vendor.subcategories.length > 3 && (
                      <span className="bg-[#1E2D4D]/5 rounded-full px-2.5 py-0.5 text-xs font-medium text-[#1E2D4D]/70">
                  ...
                  {/* Description */}
                  <p className="text-xs text-[#1E2D4D]/50 mb-4 line-clamp-2">
                    {vendor.description}
                  </p>
          ...
          <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-12 text-center">
            ...
            <p className="text-[#1E2D4D]/70 font-medium">
