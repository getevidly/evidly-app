...
import {
  Fan, CheckCircle, AlertTriangle, DollarSign, Calendar,
...
export default function FanPerformanceManagement() {
  usePageTitle('Fan Performance Management');
  ...
  const [hasRecords, setHasRecords] = useState(null); // null = loading
  ...
  // Check if any records exist for this service
  useEffect(() => {
  ...
  const fpmHistory = history.filter(r => r.service_type_code === SERVICE_CODE);
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', paddingBottom: 100 }} className="space-y-4 px-4 pt-4">
      ...
      {isEmpty && (
        <div className="rounded-lg" style={{ background: colors.white, padding: '16px', boxShadow: shadows.sm, textAlign: 'center' }}>
          ...
          <button
            ...
            style={{ padding: '8px 20px', fontSize: typography.size.sm, fontWeight: typography.weight.semibold, color: colors.white, background: colors.navy, border: 'none', borderRadius: radius.md, cursor: 'pointer' }}
          >
            Add to plan
          </button>
        </div>
          ...
          <div>
            ...
            <div className="rounded-lg" style={{ borderLeft: `3px solid ${overallStatus.border}`, background: colors.white, padding: '10px 12px', boxShadow: shadows.sm }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Fan size={16} color="#D85A30" />
                <div>
                  <p style={{ fontSize: typography.size.sm, fontWeight: typography.weight.semibold, color: colors.textPrimary }}>Exhaust Fans</p>
                  <p style={{ fontSize: typography.size.xs, color: colors.textMuted }}>Belt service, bearing lubrication, airflow verification</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {showRequest && orgId && (
        <Suspense fallback={null}>
          <RequestServiceModal isOpen onClose={() => setShowRequest(false)} locationId={locationId || ''} organizationId={orgId} defaultServiceType="FPM" vendorId={schedule?.vendor_id || undefined} vendorName={schedule?.vendor_name || undefined} />
        </Suspense>
      )}
    </div>
  );
