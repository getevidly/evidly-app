...
export default function RooftopGreaseContainment() {
  usePageTitle('Rooftop Grease Containment');
  ...
  const rgcHistory = history.filter(r => r.service_type_code === SERVICE_CODE);
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', paddingBottom: 100 }} className="space-y-4 px-4 pt-4">
          ...
          <div>
            ...
            <div className="rounded-lg" style={{ borderLeft: `3px solid ${overallStatus.border}`, background: colors.white, padding: '10px 12px', boxShadow: shadows.sm }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Shield size={16} color="#D85A30" />
                <div>
                  <p style={{ fontSize: typography.size.sm, fontWeight: typography.weight.semibold, color: colors.textPrimary }}>Rooftop Containment Apparatus</p>
                  <p style={{ fontSize: typography.size.xs, color: colors.textMuted }}>Grease capture, roof drain protection, CWA wastewater compliance</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {showRequest && orgId && (
        <Suspense fallback={null}>
          <RequestServiceModal isOpen onClose={() => setShowRequest(false)} locationId={locationId || ''} organizationId={orgId} defaultServiceType="RGC" vendorId={schedule?.vendor_id || undefined} vendorName={schedule?.vendor_name || undefined} />
        </Suspense>
      )}
    </div>
  );
