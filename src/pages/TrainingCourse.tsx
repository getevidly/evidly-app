    ...
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
      ...
      <div style={{ background: '#f9fafb', padding: '16px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {questions.map((_, i) => (
            <div key={i} style={{ width: 10, height: 10, borderRadius: 5, background: i < currentQ ? '#1E2D4D' : i === currentQ ? '#B24A2E' : '#e5e7eb' }} />
          ))}
        </div>
      </div>
      {/* Question */}
...
function CertificateView({ cert, course }: { cert: TrainingCertificate; course: TCourse }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '2px solid #B24A2E', padding: 40, textAlign: 'center', maxWidth: 600, margin: '0 auto' }}>
      <div style={{ borderBottom: '2px solid #B24A2E', paddingBottom: 24, marginBottom: 24 }}>
        <Award size={48} color="#B24A2E" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: 24 }}
          ...
          <div>
            ...
            {cert && (
              <div style={{ background: '#fffbeb', borderRadius: 10, border: '1px solid #fde68a', padding: 20, marginBottom: 16, textAlign: 'center' }}>
                ...
                <button onClick={() => setView('certificate')}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '6px 14px', borderRadius: 8, border: '1px solid #B24A2E', background: '#fff', color: '#92400e', fontSize: 13, fontWeight: 600, cursor: 'pointer', marginTop: 10, fontFamily: "'DM Sans', sans-serif" }}>
                  <Eye size={14} /> View
                </button>
              </div>
            )}
            ...
            <button onClick={() => setShowAI(!showAI)}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 16px', borderRadius: 10, border: '1px solid #B24A2E', background: showAI ? '#fffbeb' : '#fff', color: '#92400e', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 16, fontFamily: "'DM Sans', sans-serif" }}>
