...
export function AiUpgradePrompt({ feature, description, variant = 'inline', onClose }: AiUpgradePromptProps) {
  ...
  if (variant === 'banner') {
    return (
      <div
        ...
        style={{ backgroundColor: '#fdf8e8', border: '1px solid #B24A2E' }}
      >
        <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#B24A2E' }}>
    ...
    return (
      <Modal isOpen={true} onClose={onClose ?? (() => {})} size="md" closeOnBackdrop={!!onClose} className="border border-[#1E2D4D]/10 p-6 text-center">
          <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: '#fdf8e8' }}>
            <Sparkles className="h-7 w-7" style={{ color: '#B24A2E' }} />
          </div>
  ...
  return (
    ...
    >
      <Lock className="h-5 w-5 mx-auto mb-2" style={{ color: '#B24A2E' }} />
