import type { IncidentsPRPStats } from '../../hooks/incidents/useIncidentsPRPStats';

const PILLAR_LABEL = 'text-[10px] uppercase font-bold tracking-[0.12em]';

interface IncidentsPRPBandProps {
  stats: IncidentsPRPStats;
  loading: boolean;
  onFilterPredict?: () => void;
  onFilterProve?: () => void;
}

export function IncidentsPRPBand({
  stats,
  loading,
  onFilterPredict,
  onFilterProve,
}: IncidentsPRPBandProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-[#6B7F96]">
                {stats.agingCount} incident{stats.agingCount !== 1 ? 's' : ''} aging
                beyond 24h. {stats.patternCount} recurring pattern{stats.patternCount !== 1 ? 's' : ''} flagged.
              </p>
      ...
      >
        ...
        <div className="mt-3 pt-3" style={{ borderTop: '1px dashed #E2DDD4' }}>
          <p className="text-[11px] text-[#6B7F96] leading-relaxed">
            Incident-level exposure scoring is not yet available. When enabled,
            this card will show the estimated liability window for open incidents.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="text-[11px] text-[#8A93A6] mt-0.5">Resolved this week</div>
        {loading ? (
          <div className="h-8 w-16 mt-2 rounded bg-[#1E2D4D]/5 animate-pulse" />
        ) : (
        ...
        <div className="mt-3 pt-3" style={{ borderTop: '1px dashed #E2DDD4' }}>
          {loading ? (
            ...
            <>
              <p className="text-[11px] text-[#6B7F96]">
                Incidents closed and documented this week.
              </p>
