...
const ROWS = [
  {
    ...
    bg: '#EEEDFE',
    accent: '#534AB7',
    format: (m: ShiftPRPMetrics) => `${m.predict} items coming due in next 14 days`,
  },
  {
    key: 'reduce' as const,
    label: 'Reduce',
    ...
    bg: '#FAEEDA',
    accent: '#BA7517',
    format: (m: ShiftPRPMetrics) => `${m.reduce} active corrective actions`,
  },
  {
    key: 'prove' as const,
    label: 'Prove',
...
export function ShiftPRPBlock({ metrics, loading }: ShiftPRPBlockProps) {
  return (
