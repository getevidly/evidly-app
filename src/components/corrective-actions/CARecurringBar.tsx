...
import type { CARecurringPattern } from '../../hooks/corrective-actions/useCARecurringPatterns';
...
function formatRootCause(rc: string): string {
  return rc.length > 60 ? rc.slice(0, 57) + '...' : rc;
}

function PatternCard({
  pattern,
}: {
  pattern: CARecurringPattern;
}) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold" style={{ color: '#c2731a' }}>
          Recurring root cause identified
        </p>
        <p className="text-[12px] text-[#1E2D4D]/70 mt-0.5">
          {categoryLabel} actions reported {pattern.count30d} time{pattern.count30d !== 1 ? 's' : ''} in
          the past 30 days.
          {rootCauseLabel && (
...
export function CARecurringBar({
  patterns,
}: {
  patterns: CARecurringPattern[];
}) {
    ...
    <div className="space-y-2">
      ...
      {remaining > 0 && (
        <p className="text-[11px] font-medium text-[#c2731a] pl-1">
          View all {patterns.length} recurring root causes →
        </p>
      )}
    </div>
  );
