  ...
  <div style={{
    ...
    {data.complianceMatrix.map((loc: any) => {
      ...
      return (
        <div key={loc.locationId} style={{
          borderBottom: '1px solid #D1D9E6', paddingBottom: '16px', marginBottom: '16px',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
            <p style={{ color: '#0B1628', fontSize: '13px', fontWeight: 600, margin: 0, fontFamily: 'system-ui' }}>{loc.locationName}</p>
            {correlation && (
              <span style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {[
              ...
              <div key={i} style={{ background: '#EEF1F7', borderRadius: '8px', padding: '12px' }}>
                <p style={{ color: '#3D5068', fontSize: '11px', margin: '0 0 4px', fontFamily: 'system-ui' }}>{m.label}</p>
                <p style={{ color: m.color, fontSize: '22px', fontWeight: 800, margin: '0 0 2px', fontFamily: 'monospace' }}>{m.value}</p>
                <p style={{ color: '#3D5068', fontSize: '10px', margin: 0, fontFamily: 'system-ui' }}>{m.benchmark}</p>
              </div>
            ))}
          </div>
          {correlation && (
            <div style={{
              background: '#EEF1F7', borderRadius: '6px', padding: '10px 12px', marginTop: '10px',
    ...
    <div style={{
      ...
      <p style={{ color: '#3D5068', fontSize: '12px', margin: 0, lineHeight: 1.5, fontFamily: 'system-ui' }}>
        ...
        {' '}Your checklist completion: <strong style={{ color: '#0B1628' }}>{Math.round(data.orgMetrics.avgChecklistCompletion * 100)}%</strong> vs. industry average of {Math.round(data.industryBenchmarks.avgChecklistCompletion * 100)}% and top decile of {Math.round(data.industryBenchmarks.topDecileChecklistCompletion * 100)}%.
      </p>
    </div>
  </div>
);
