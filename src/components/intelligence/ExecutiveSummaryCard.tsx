...
const viewPrompts: Record<string, string> = {
  ...
  risk: `You are a regulatory risk advisor. Analyze this commercial kitchen compliance data and write a 3-paragraph executive briefing for a VP of Risk or General Counsel.
    ...
    Paragraph 2: Specific threats — permit expirations, failed facility safety items, open violations, reinspection requirements, and their potential legal/regulatory consequences.
    Paragraph 3: Risk mitigation priorities ranked by urgency and potential liability reduction.
    Be precise about regulatory citations. Reference NFPA 96 Table 12.4 where relevant.`,
...
const demoSummaries: Record<string, string> = { // demo
  operations: `Your Organization's compliance posture shows a clear bifurcation: Location 1 operates at 96% checklist completion with an improving trajectory and zero incidents — a model location. However, the organizational average of 74% checklist completion is pulled down significantly by Location 2 (71%) and Location 3 (54%), both on declining trajectories over the past 90 days. Temp log completion mirrors this pattern at 76% org-wide, indicating the compliance gaps are systemic at underperforming locations rather than task-specific. // demo

The most urgent operational risk is Location 3, which requires immediate intervention. With 6 open items, a reinspection window closing March 1, and checklist completion at 54% and falling, this location is on track for a failed reinspection. Location 2's facility safety failure compounds the organizational risk — two of three locations now carry active compliance deficiencies. The declining trends at both locations suggest the root cause has not been addressed. // demo

Three strategic priorities: First, deploy management support to Location 3 before the March 1 reinspection — a reinspection failure at current trajectory is near-certain without intervention. Second, remediate Location 2's facility safety failure and address the 18-day hood cleaning certificate expiration before it compounds into a multi-violation scenario. Third, replicate Location 1's operational model (staffing stability, checklist discipline, management oversight) as the organizational standard. The compliance gap between your best and worst locations represents a 42-point spread — closing that gap by even half would materially change the risk profile.`, // demo

  risk: `Your Organization faces $520,500 in estimated annual risk exposure, concentrated overwhelmingly at two locations. Location 3 represents $385,000 of that exposure — a single location carrying 74% of the organization's total regulatory and legal risk. This location has an active "Action Required" food safety status, a failed facility safety inspection, a health permit expiring in 7 days, and a hood cleaning certificate expiring in 3 days. If the hood cleaning certificate expires while the facility safety failure remains unresolved, the AHJ has grounds for immediate closure under NFPA 96 enforcement. Location 2 adds $127,000 in exposure with its own facility safety failure and 18-day hood cleaning certificate expiration. // demo

The specific threat chain at Location 3 is: hood cleaning certificate expires in 3 days → facility safety non-compliance confirmed → health permit expires in 7 days → reinspection due March 1 with 6 open items → if reinspection fails, the AHJ may issue a closure notice. Each of these events compounds the next. Separately, Location 2's fire suppression inspection expires in 45 days — if that location enters reinspection with both a facility safety failure and an expired fire suppression certificate, the probability of enforcement action increases substantially. Two active permit expirations within 30 days across the organization is a critical documentation gap. // demo

Priority 1: Renew Location 3's hood cleaning certificate within 48 hours — this is the single highest-impact action available. Priority 2: Resolve Location 3's food safety violations before the March 1 reinspection. Priority 3: Address Location 2's facility safety failure and schedule hood cleaning certificate renewal. These three actions would reduce total organizational risk exposure by an estimated 60-70%.`, // demo

  financial: `Total quantified risk exposure across Your Organization stands at $520,500 annually, against estimated compliance savings of $341,000. The net financial case for compliance investment is not the savings alone — it is the avoidance of the $520,500 exposure. Location 3 alone carries $385,000 in risk exposure: potential closure costs (revenue loss at $2,500-5,000/day for 3-14 days), reinspection fees, emergency remediation premiums, legal consultation for enforcement defense, and insurance carrier dispute grounds if an incident occurs with expired permits and active violations. The 3-day hood cleaning certificate expiration creates immediate insurance vulnerability. // demo

The ROI of EvidLY's compliance platform is most visible at Location 1: $42,000 in estimated annual savings from maintained compliance, zero incidents, and a documented record that supports favorable insurance positioning. That location demonstrates what systematic compliance management produces. The gap between Location 1 ($8,500 exposure) and Location 3 ($385,000 exposure) is a 45:1 ratio — the difference is not the tool, it is the operational discipline the tool enables. Investing in operational support for the two underperforming locations would yield an estimated 15:1 return on investment.

The highest-return financial actions: (1) Location 3 remediation — estimated $2,000-8,000 investment to address facility safety and permit renewals, against $385,000 in exposure reduction. (2) Location 2 facility safety resolution — estimated $2,000-5,000 investment against $127,000 exposure. (3) At next insurance renewal, present Location 1's EvidLY compliance documentation as the basis for premium renegotiation — carriers increasingly factor digital compliance documentation into underwriting decisions.`, // demo

  people: `The staffing-compliance correlation across Your Organization is striking and quantifiable. Location 1 operates at 12% annual turnover with 96% checklist completion — your strongest compliance location is also your most stable workforce. Location 2 shows 34% turnover with 71% checklist completion, and Location 3 has 51% turnover with 54% checklist completion. The organization-wide pattern is clear: each 10-point increase in turnover rate correlates with approximately 10-12 points of checklist completion decline. Location 3's 51% turnover is the single strongest predictor of its compliance failure trajectory. // demo

The operational chain is direct: high turnover means new staff who lack training → untrained staff miss checklist items and temperature logs → open compliance items accumulate → inspection readiness declines → enforcement risk increases → operational disruption from enforcement further increases turnover. Location 3 appears to be in the negative feedback loop phase of this cycle — turnover is both causing and being caused by operational instability. The 4 incidents in 90 days at that location likely accelerated the cycle. Each new hire requires approximately 3 weeks to reach compliance proficiency, meaning Location 3 is perpetually operating with a partially-trained workforce. // demo

Three people-strategy investments would most improve compliance outcomes: First, conduct a retention diagnostic at Location 3 — identify whether the turnover driver is compensation, management, working conditions, or scheduling, and address the root cause. Reducing turnover from 51% to 30% would be estimated to improve checklist completion by 15-20 points. Second, implement a structured 3-week onboarding compliance checklist for all new hires across all locations — this reduces the proficiency gap during the high-risk transition period. Third, assign a compliance mentor at Location 2 and Location 3 — a designated team member responsible for daily checklist oversight until completion rates stabilize above 85%.`, // demo
};
  ...
  const generateSummary = async () => {
    ...
    const dataContext = JSON.stringify({
      ...
      complianceMatrix: data.complianceMatrix.map((l: any) => ({
        ...
        facilitySafetyVerdict: l.facilitySafetyVerdict,
