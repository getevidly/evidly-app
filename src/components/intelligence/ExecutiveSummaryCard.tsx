...
const demoSummaries: Record<string, string> = { // demo
  operations: `Your Organization's compliance posture shows a clear bifurcation: Downtown Kitchen operates at 96% checklist completion with an improving trajectory and zero incidents — a model location. However, the organizational average of 74% checklist completion is pulled down significantly by Airport Cafe (71%) and University Dining (54%), both on declining trajectories over the past 90 days. Temp log completion mirrors this pattern at 76% org-wide, indicating the compliance gaps are systemic at underperforming locations rather than task-specific. // demo

The most urgent operational risk is University Dining, which requires immediate intervention. With 6 open items, a reinspection window closing March 1, and checklist completion at 54% and falling, this location is on track for a failed reinspection. Airport Cafe's facility safety failure compounds the organizational risk — two of three locations now carry active compliance deficiencies. The declining trends at both locations suggest the root cause has not been addressed. // demo

Three strategic priorities: First, deploy management support to University Dining before the March 1 reinspection — a reinspection failure at current trajectory is near-certain without intervention. Second, remediate Airport Cafe's facility safety failure and address the 18-day hood cleaning certificate expiration before it compounds into a multi-violation scenario. Third, replicate Downtown Kitchen's operational model (staffing stability, checklist discipline, management oversight) as the organizational standard. The compliance gap between your best and worst locations represents a 42-point spread — closing that gap by even half would materially change the risk profile.`, // demo

  risk: `Your Organization faces $520,500 in estimated annual risk exposure, concentrated overwhelmingly at two locations. University Dining represents $385,000 of that exposure — a single location carrying 74% of the organization's total regulatory and legal risk. This location has an active "Action Required" food safety status, a failed facility safety inspection, a health permit expiring in 7 days, and a hood cleaning certificate expiring in 3 days. If the hood cleaning certificate expires while the facility safety failure remains unresolved, the AHJ has grounds for immediate closure under NFPA 96 (2024) enforcement. Airport Cafe adds $127,000 in exposure with its own facility safety failure and 18-day hood cleaning certificate expiration. // demo

The specific threat chain at University Dining is: hood cleaning certificate expires in 3 days → facility safety non-compliance confirmed → health permit expires in 7 days → reinspection due March 1 with 6 open items → if reinspection fails, the AHJ may issue a closure notice. Each of these events compounds the next. Separately, Airport Cafe's fire suppression inspection expires in 45 days — if that location enters reinspection with both a facility safety failure and an expired fire suppression certificate, the probability of enforcement action increases substantially. Two active permit expirations within 30 days across the organization is a critical documentation gap. // demo

Priority 1: Renew University Dining's hood cleaning certificate within 48 hours — this is the single highest-impact action available. Priority 2: Resolve University Dining's food safety violations before the March 1 reinspection. Priority 3: Address Airport Cafe's facility safety failure and schedule hood cleaning certificate renewal. These three actions would reduce total organizational risk exposure by an estimated 60-70%.`, // demo
