...
<html>
<head>
  <title>EvidLY Business Intelligence Report — ${data.orgName}</title>
  <style>
    body { font-family: system-ui, -apple-system, 'Segoe UI', sans-serif; max-width: 800px; margin: 0 auto; padding: 40px; color: #1e3a5f; }
    h1 { font-size: 22px; color: #1e3a5f; margin-bottom: 4px; }
    h2 { font-size: 16px; color: #2d4a6e; margin-top: 28px; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; }
...
<body>
  ...
  <p class="subtitle">${data.orgName} — ${data.period} Analysis — Generated ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
  ...
  ${data.complianceMatrix.map((loc: any) => `
    <div class="location-row">
      ...
      <div style="text-align:right">
        ...
        <span style="font-size:12px;color:#94a3b8">Exposure: $${loc.estimatedRiskExposure.toLocaleString()}</span>
      </div>
    </div>
  `).join('')}
  ...
  )).join('') || '<p style="color:#94a3b8;font-size:13px">No permits expiring in the next 30 days.</p>'}

  <div class="footer">
    Generated by EvidLY Business Intelligence · ${new Date().toISOString()}<br/>
    This report contains estimates based on compliance data. Verify food safety requirements with your Environmental Health Department (EHD) and fire safety requirements with your fire Authority Having Jurisdiction (AHJ).
  </div>
</body>
</html>`;
