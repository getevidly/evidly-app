...
export default function AuthModal({ mode, onClose, onSwitchMode }: AuthModalProps) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { error: authError } = await signUp(email, password, fullName, '', orgName);
      if (authError) { setError(authError.message); }
  ...
  return (
    <Modal isOpen={true} onClose={onClose} size="sm">
      <div style={{ position:'relative' }}>
        <button onClick={onClose} style={{ position:'absolute',top:'16px',right:'16px',background:'none',border:'none',cursor:'pointer',padding:'6px',borderRadius:'50%',zIndex:1 }} aria-label="Close">
        ...
        <div style={{ padding:'32px 32px 16px',textAlign:'center' }}>
          <div style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',marginBottom:'16px' }}>
            ...
            <span style={{ fontFamily:'Syne, system-ui, sans-serif',fontWeight:800,fontSize:'20px' }}>
              <span style={{ color:'#B24A2E' }}>Evid</span><span style={{ color:'#1E2D4D' }}>LY</span>
            </span>
          </div>
