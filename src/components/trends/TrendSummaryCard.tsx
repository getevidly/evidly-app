...
export function TrendSummaryCard({ label, analysis, color = '#1E2D4D' }: Props) {
  return (
    <div className="bg-white rounded-xl border border-[#1E2D4D]/10 p-4">
      <p className="text-xs font-medium text-[#1E2D4D]/50 uppercase tracking-wide mb-1">{label}</p>
      <div className="flex items-end gap-3">
        <span className="text-3xl font-bold tracking-tight" style={{ color }}>{analysis.currentValue}</span>
        <TrendDirectionBadge direction={analysis.direction} delta={analysis.periodDelta} />
      </div>
      <div className="mt-2 flex items-center gap-3 text-xs text-[#1E2D4D]/50">
