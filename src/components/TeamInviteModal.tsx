...
const ROLE_OPTIONS: { value: string; label: string; description: string }[] = [
  ...
  { value: 'compliance_manager', label: 'Compliance Officer', description: 'Regulatory compliance, inspections, reports' },
        ...
        <form onSubmit={isBulk ? handleBulkSubmit : handleSingleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={handleClose}
              className="flex-1 px-4 py-2 border-2 border-[#1E2D4D] text-[#1E2D4D] rounded-xl hover:bg-[#FAF7F0] transition-colors bg-white"
            >
              Cancel
            </button>
            <button
