...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch jurisdiction grade and violation detail for this location
    let scoreSnapshot: any = null;
    if (payload.location_id) {
      const { data } = await supabase
        .from("compliance_score_snapshots")
        .select("id, jurisdiction_grade, score_date, model_version")
    ...
    // Fetch per-violation detail for the location
    let violationDetail: any[] = [];
    if (payload.location_id) {
      const { data: violations } = await supabase
        .from("violations")
        .select("id, code, description, severity, status, observed_at")
        .eq("location_id", payload.location_id)
        .eq("organization_id", orgId)
    ...
    const manifest = {
      ...
      // Jurisdiction grade and per-violation detail (no manufactured scores)
      jurisdiction_compliance: scoreSnapshot ? {
        jurisdiction_grade: scoreSnapshot.jurisdiction_grade,
        score_date: scoreSnapshot.score_date,
        model_version: scoreSnapshot.model_version,
        score_snapshot_id: scoreSnapshot.id,
      } : null,
      violations: violationDetail.map((v: any) => ({
        id: v.id,
        code: v.code,
    ...
    try {
      ...
      // Jurisdiction Grade section
      let yPos = 575;
      if (scoreSnapshot) {
        cover.drawText("Jurisdiction Compliance", { x: 50, y: yPos, size: 18, font: boldFont, color: brandBlue });
        yPos -= 28;
        cover.drawText(`Grade: ${scoreSnapshot.jurisdiction_grade ?? "N/A"}`, { x: 60, y: yPos, size: 14, font: boldFont });
        yPos -= 20;
        cover.drawText(`Open Violations: ${violationDetail.filter((v: any) => v.status === "open").length}`, { x: 60, y: yPos, size: 12, font });
        yPos -= 18;
        cover.drawText(`Total Violations on Record: ${violationDetail.length}`, { x: 60, y: yPos, size: 12, font });
        yPos -= 18;
