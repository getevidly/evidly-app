...
Deno.serve(async (_req: Request) => {
  ...
  try {
    ...
    if (taskErr) {
      return Response.json({ error: "task query failed", detail: taskErr });
    }

    if (!tasks || tasks.length === 0) {
      return Response.json({ done: true, message: "no pending tasks" });
    }
    ...
    if (claimErr) {
      return Response.json({ error: "claim failed", detail: claimErr });
    }

    // ── Auth: GET homepage, capture __ECO_SESS cookie ──────────────
    ...
    if (!guid) {
      await supabase.from("inspection_crawl_tasks").update({
        status: "error", last_error: "Could not extract PressAgentOid from homepage",
      }).eq("id", task.id);
      return Response.json({ error: "Could not extract PressAgentOid", taskId: task.id });
    }
    ...
    try {
      ...
      await supabase.from("inspection_crawl_tasks").update({
        status: "error", last_error: "searchFacilities non-JSON: " + text.slice(0, 500),
      }).eq("id", task.id);
      return Response.json({ error: "searchFacilities non-JSON", taskId: task.id });
    }
    ...
    if (facResults.length === 50) {
      ...
      if (task.search_kind === "zip") {
        ...
        if (spawnErr) {
          await supabase.from("inspection_crawl_tasks").update({
            status: "error", last_error: "spawn failed: " + JSON.stringify(spawnErr),
          }).eq("id", task.id);
          return Response.json({ error: "spawn failed", detail: spawnErr, taskId: task.id });
        }
        ...
        return Response.json({
          ...
          message: "Zip truncated at 50; spawned 26 name_prefix sub-tasks",
        });
      }
    ...
    for (const fac of facResults) {
      ...
      if (facErr) {
        await supabase.from("inspection_crawl_tasks").update({
          status: "error", last_error: "facility upsert: " + JSON.stringify(facErr),
        }).eq("id", task.id);
        return Response.json({ error: "facility upsert failed", detail: facErr, facKey });
      }
      ...
      for (const prog of progArr) {
        ...
        for (const insp of inspArr) {
          ...
          if (inspErr) {
            await supabase.from("inspection_crawl_tasks").update({
              status: "error", last_error: "inspection upsert: " + JSON.stringify(inspErr),
            }).eq("id", task.id);
            return Response.json({ error: "inspection upsert failed", detail: inspErr, inspOid });
          }
          ...
          for (const vio of violations) {
            ...
            if (vioErr) {
              await supabase.from("inspection_crawl_tasks").update({
                status: "error", last_error: "violation upsert: " + JSON.stringify(vioErr),
              }).eq("id", task.id);
              return Response.json({ error: "violation upsert failed", detail: vioErr, vioOid });
            }
    ...
    return Response.json({
      ...
      search_kind: task.search_kind,
      search_value: task.search_value,
      facilitiesFound: facResults.length,
      facilitiesWritten,
      inspectionsWritten,
      violationsWritten,
      truncated,
      tasksSpawned,
      remainingPending: remaining ?? 0,
      ...(truncated && task.search_kind === "name_prefix"
        ? { warning: "name_prefix truncated at 50 — needs deeper subdivision" }
        : {}),
    });
  } catch (err) {
    // Attempt to mark the task as errored
    try {
      ...
      if (tasks && tasks[0]) {
        await supabase.from("inspection_crawl_tasks").update({
          status: "error", last_error: String(err),
        }).eq("id", tasks[0].id);
      }
    } catch { /* best effort */ }
