...
async function calculateFireFactors(
  ...
  const factors: Factor[] = [
    { name: "Hood cleaning current per NFPA 96 schedule", score: hoodScore, weight: 0.18, status: factorStatus(hoodScore), detail: `Hood cleaning: ${vendorDetail("Hood Cleaning")}`, reference: "NFPA 96", category: "fire", improvement: "Schedule hood cleaning service" },
    { name: "Fire suppression system inspected (semi-annual)", score: fireScore, weight: 0.16, status: factorStatus(fireScore), detail: `Fire suppression: ${vendorDetail("Fire Suppression")}`, reference: "NFPA 17A", category: "fire", improvement: "Schedule fire suppression inspection" },
    { name: "Fire extinguisher inspected (annual)", score: extScore, weight: 0.14, status: factorStatus(extScore), detail: extScore >= 90 ? "Annual inspection current" : "Inspection approaching or overdue", reference: "NFPA 10", category: "fire", improvement: "Schedule extinguisher inspection" },
    { name: "Monthly visual checks documented", score: Math.min(100, Math.round(hoodScore * 1.05)), weight: 0.10, status: factorStatus(Math.round(hoodScore * 1.05)), detail: hoodScore >= 80 ? "Visual inspection logs up to date" : "Documentation gaps in visual checks", reference: "NFPA 10 §7.2", category: "fire", improvement: "Complete monthly visual inspection checklist" },
    { name: "Cleaning to bare metal verified", score: hoodScore >= 90 ? 100 : hoodScore >= 50 ? 70 : 30, weight: 0.10, status: factorStatus(hoodScore >= 90 ? 100 : hoodScore >= 50 ? 70 : 30), detail: hoodScore >= 90 ? "Bare metal cleaning documented" : "Cleaning verification incomplete", reference: "NFPA 96 §11.4", category: "fire", improvement: "Request bare metal cleaning verification from vendor" },
    { name: "Automatic fuel/electric shutoff tested", score: greaseScore, weight: 0.10, status: factorStatus(greaseScore), detail: greaseScore >= 80 ? "Shutoff systems tested" : "Shutoff testing overdue", reference: "NFPA 96 §10.1", category: "fire", improvement: "Schedule shutoff system test" },
    { name: "Manual pull station accessible and tested", score: Math.min(100, Math.round(fireScore * 1.02)), weight: 0.08, status: factorStatus(Math.round(fireScore * 1.02)), detail: fireScore >= 80 ? "Pull station accessible and tested" : "Pull station testing needed", reference: "NFPA 17A", category: "fire", improvement: "Test manual pull station" },
    { name: "Fire alarm system current", score: Math.min(100, Math.round((fireScore + extScore) / 2 * 1.05)), weight: 0.08, status: factorStatus(Math.round((fireScore + extScore) / 2 * 1.05)), detail: "Fire alarm monitoring status", reference: "NFPA 72", category: "fire", improvement: "Schedule fire alarm inspection" },
    { name: "Documentation on file and accessible", score: Math.round(hoodScore * 0.95), weight: 0.06, status: factorStatus(Math.round(hoodScore * 0.95)), detail: hoodScore >= 80 ? "All fire safety records on file" : "Documentation gaps detected", reference: "NFPA 96 §14.2", category: "fire", improvement: "Upload missing fire safety documentation" },
  ];
