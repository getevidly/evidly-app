...
CREATE INDEX IF NOT EXISTS idx_k2c_codes_org ON k2c_referral_codes(organization_id);
CREATE INDEX IF NOT EXISTS idx_k2c_codes_code ON k2c_referral_codes(code);

ALTER TABLE k2c_referral_codes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own org referral codes" ON k2c_referral_codes;
CREATE POLICY "Users can view own org referral codes"
  ON k2c_referral_codes FOR SELECT TO authenticated
  USING (organization_id IN (SELECT organization_id FROM user_profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert own org referral codes" ON k2c_referral_codes;
CREATE POLICY "Users can insert own org referral codes"
  ON k2c_referral_codes FOR INSERT TO authenticated
  WITH CHECK (organization_id IN (SELECT organization_id FROM user_profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Service role full access k2c_referral_codes" ON k2c_referral_codes;
...
CREATE INDEX IF NOT EXISTS idx_k2c_referrals_org ON k2c_referrals(referrer_org_id, invited_at DESC);
CREATE INDEX IF NOT EXISTS idx_k2c_referrals_code ON k2c_referrals(referrer_code);
CREATE INDEX IF NOT EXISTS idx_k2c_referrals_email ON k2c_referrals(email);
CREATE INDEX IF NOT EXISTS idx_k2c_referrals_status ON k2c_referrals(status);

ALTER TABLE k2c_referrals ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own org referrals" ON k2c_referrals;
...
DROP POLICY IF EXISTS "Users can insert own org referrals" ON k2c_referrals;
...
DROP POLICY IF EXISTS "Users can update own org referrals" ON k2c_referrals;
CREATE POLICY "Users can update own org referrals"
  ON k2c_referrals FOR UPDATE TO authenticated
  USING (referrer_org_id IN (SELECT organization_id FROM user_profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Service role full access k2c_referrals" ON k2c_referrals;
