...
DECLARE
  ...
  v_address_count int;
  v_website_count int;
  v_poc_name_count int;
  v_poc_title_count int;
  v_fire_phone_count int;
  v_fire_website_count int;
  v_ds_jsonb int;
  v_ds_firecrawl int;
  v_ds_unverified int;
  v_merced_phone text;
  v_alpine_poc text;
  v_amador_ds text;
  v_sutter_website text;
  v_colusa_website text;
  v_colusa_ds text;
BEGIN
...
WHERE state = 'CA' AND county = 'Yuba' AND city IS NULL;
  ...
  SELECT COUNT(*) INTO v_ca_count FROM jurisdictions WHERE state = 'CA';
  IF v_ca_count != 62 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: CA row count expected 62, got %', v_ca_count;
  END IF;

  -- === ASSERTION 2: Cell counts per column ===
  SELECT COUNT(*) INTO v_phone_count FROM jurisdictions WHERE state = 'CA' AND agency_phone IS NOT NULL;
  IF v_phone_count != 47 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: agency_phone non-null expected 47, got %', v_phone_count;
  END IF;

  SELECT COUNT(*) INTO v_fax_count FROM jurisdictions WHERE state = 'CA' AND agency_fax IS NOT NULL;
  IF v_fax_count != 21 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: agency_fax non-null expected 21, got %', v_fax_count;
  END IF;

  SELECT COUNT(*) INTO v_email_count FROM jurisdictions WHERE state = 'CA' AND agency_email IS NOT NULL;
  IF v_email_count != 21 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: agency_email non-null expected 21, got %', v_email_count;
  END IF;

  SELECT COUNT(*) INTO v_address_count FROM jurisdictions WHERE state = 'CA' AND agency_address IS NOT NULL;
  IF v_address_count != 44 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: agency_address non-null expected 44, got %', v_address_count;
  END IF;

  SELECT COUNT(*) INTO v_website_count FROM jurisdictions WHERE state = 'CA' AND agency_website IS NOT NULL;
  IF v_website_count != 57 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: agency_website non-null expected 57, got %', v_website_count;
  END IF;

  SELECT COUNT(*) INTO v_poc_name_count FROM jurisdictions WHERE state = 'CA' AND poc_name IS NOT NULL;
  IF v_poc_name_count != 10 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: poc_name non-null expected 10, got %', v_poc_name_count;
  END IF;

  SELECT COUNT(*) INTO v_poc_title_count FROM jurisdictions WHERE state = 'CA' AND poc_title IS NOT NULL;
  IF v_poc_title_count != 10 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: poc_title non-null expected 10, got %', v_poc_title_count;
  END IF;

  SELECT COUNT(*) INTO v_fire_phone_count FROM jurisdictions WHERE state = 'CA' AND fire_ahj_phone IS NOT NULL;
  IF v_fire_phone_count != 56 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: fire_ahj_phone non-null expected 56, got %', v_fire_phone_count;
  END IF;

  SELECT COUNT(*) INTO v_fire_website_count FROM jurisdictions WHERE state = 'CA' AND fire_ahj_website IS NOT NULL;
  IF v_fire_website_count != 56 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: fire_ahj_website non-null expected 56, got %', v_fire_website_count;
  END IF;

  -- === ASSERTION 3: contact_data_source distribution ===
  SELECT COUNT(*) INTO v_ds_jsonb FROM jurisdictions WHERE state = 'CA' AND contact_data_source = 'jsonb_existing';
  SELECT COUNT(*) INTO v_ds_firecrawl FROM jurisdictions WHERE state = 'CA' AND contact_data_source = 'firecrawl_pending_review';
  SELECT COUNT(*) INTO v_ds_unverified FROM jurisdictions WHERE state = 'CA' AND contact_data_source = 'unverified';

  IF v_ds_jsonb != 40 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source jsonb_existing expected 40, got %', v_ds_jsonb;
  END IF;
  IF v_ds_firecrawl != 10 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source firecrawl_pending_review expected 10, got %', v_ds_firecrawl;
  END IF;
  IF v_ds_unverified != 12 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source unverified expected 12, got %', v_ds_unverified;
  END IF;

  -- === ASSERTION 4: Spot checks ===
  SELECT agency_phone INTO v_merced_phone FROM jurisdictions WHERE state = 'CA' AND county = 'Merced' AND city IS NULL;
  IF v_merced_phone != '209-381-1100' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Merced agency_phone expected 209-381-1100, got %', v_merced_phone;
  END IF;

  SELECT poc_name INTO v_alpine_poc FROM jurisdictions WHERE state = 'CA' AND county = 'Alpine' AND city IS NULL;
  IF v_alpine_poc != 'Dennis Lampson' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Alpine poc_name expected Dennis Lampson, got %', v_alpine_poc;
  END IF;

  SELECT contact_data_source INTO v_amador_ds FROM jurisdictions WHERE state = 'CA' AND county = 'Amador' AND city IS NULL;
  IF v_amador_ds != 'firecrawl_pending_review' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Amador contact_data_source expected firecrawl_pending_review, got %', v_amador_ds;
  END IF;

  SELECT agency_website INTO v_sutter_website FROM jurisdictions WHERE state = 'CA' AND county = 'Sutter' AND city IS NULL;
  IF v_sutter_website != 'https://www.sutter.gov/government/county-departments/development-services/environmental-health/food' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Sutter agency_website mismatch, got %', v_sutter_website;
  END IF;

  SELECT agency_website, contact_data_source INTO v_colusa_website, v_colusa_ds FROM jurisdictions WHERE state = 'CA' AND county = 'Colusa' AND city IS NULL;
  IF v_colusa_website IS NULL OR v_colusa_website = '' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Colusa agency_website should be filled, got NULL';
  END IF;
  IF v_colusa_ds != 'unverified' THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Colusa contact_data_source expected unverified, got %', v_colusa_ds;
  END IF;

  RAISE NOTICE 'ALL ASSERTIONS PASSED. Transaction committed.';
END;
995;
