...
WHERE state = 'CA' AND county = 'Yuba' AND city IS NULL;
  ...
  IF v_ds_jsonb != 42 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source jsonb_existing expected 42, got %', v_ds_jsonb;
  END IF;
  IF v_ds_firecrawl != 6 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source firecrawl_pending_review expected 6, got %', v_ds_firecrawl;
  END IF;
  IF v_ds_unverified != 14 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: contact_data_source unverified expected 14, got %', v_ds_unverified;
  END IF;
